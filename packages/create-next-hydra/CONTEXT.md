...
**Workspace Definition**: A version-controlled request for a named Development Workspace, containing its Workspace Selection and optional local development settings. Its stable name identifies its local application hosts and deployment directory. App-local deployment configuration may be committed alongside it; materialized manifests are not part of the definition. _Avoid_: Generated app, application profile, ownership receipt
