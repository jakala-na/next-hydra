...
The Workspace Composition context describes the selectable parts of a Next Hydra workspace and the units used to materialize them.
...
**Provider**:
A selectable implementation that fills one Provider Slot and may resolve to multiple Install Units.
...
**Add-on**:
An optional composition selection that may resolve to multiple Install Units and is valid only when its compatibility requirements are satisfied by the complete selected stack.
...
_Avoid_: Registry URL, repository path, Install Unit name

**Selection Definition**:
The Provider- or Add-on-owned metadata that identifies a selection, declares its compatibility, references its Install Units, and describes its Generated Adapters.
...
**Install Unit**:
A root-scoped code contribution that materializes one part of a selected Provider or Add-on. Install Units are implementation units, not necessarily user-facing choices.
_Avoid_: Provider, Add-on, bundle

**Contribution Source**:
The canonical code maintained for an Install Unit in its normal development location. Registry metadata may be colocated with it, but the code is not copied into a separate registry source tree.
_Avoid_: Template copy, registry copy

**Source Registry**:
The standard ShadCN `registry.json` structure used to distribute Contribution Sources. Next Hydra keeps a root registry that includes package- and application-level `registry.json` files beside the code they describe. Public GitHub and local development resolve these source files directly; v1 does not require checked-in content-inlined registry output.
...
**Generated Adapter**:
Framework-required application glue derived deterministically from a Composition Plan rather than maintained as a second source template.
_Avoid_: Provider implementation, template copy

**Additive Installation**:
A customer-approved materialization that creates missing targets, skips identical targets, and treats changed targets as explicit conflicts without inferring ownership or removing code.
_Avoid_: Recomposition, synchronization, provider switch, Customer Workspace upgrade

**Compatibility Declaration**:
A Provider- or Add-on-owned statement naming the concrete Providers or Add-ons it requires or conflicts with. It must be satisfied before the composition can be materialized; Workspace Composition validates declarations but does not centrally re-author them.
_Avoid_: Central compatibility matrix, Install Unit compatibility
