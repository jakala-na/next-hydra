...
- HTTP and Next request adapters construct the same transport-neutral **Commerce Request Input** from verified authentication, Store selection, Business Unit selection, and anonymous Cart possession. `CommerceApp.layer` composes stable provider Services, while `CommerceApp.provide(request)` composes request-scoped `CommerceContext`, `CurrentCart`, `AddressBook`, `ProductDiscovery`, and `CheckoutSession` Layers around a caller's Effect. The web app owns one module-level `ManagedRuntime` and exposes `NextCommerce.build` as its terminal Next boundary; the HTTP adapter lets its outer Effect HTTP handler own the Layer lifecycle. Callers invoke named Service methods and map typed errors to transport responses; no Contact, Address Book, or Delivery Details operation accepts context or scope.
