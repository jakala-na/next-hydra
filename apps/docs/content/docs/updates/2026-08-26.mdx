...
The new `add` command brings the ShadCN copy-and-own workflow to an existing project:
...
Projects created before this update are not rewritten automatically. They can adopt additive registry items with `add`; provider changes in an existing project still require manual reconciliation or a newly scaffolded composition.
