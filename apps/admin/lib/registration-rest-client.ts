...
export const makeRegistrationRestClient = Effect.fn(
  "AdminRegistrationRestClient.make"
  ...
  return yield* HttpApiClient.make(RegistrationHttpApi, {
    ...
    Effect.map((client) => ({
      registrations: {
        ...
        revokeInvitation: (
          request: Parameters<
            typeof client.registrations.revokeInvitation
          >[number]
        ) =>
          exposeRegistrationResponseError(
            client.registrations.revokeInvitation(request)
          ),
      },
    })),
