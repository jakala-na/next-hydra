...
Start the Portless-managed frontend in DDEV-compatible mode from the installed workspace root (not the source-only maintainer checkout):
...
Browser previews use the configured Portless HTTPS origin in customer and development workspaces alike. The Drupal package's `dev:web` command asks Portless to use application port 3001 and starts Next.js on `0.0.0.0`, so Drupal can reach it directly at `http://host.docker.internal:3001/api/revalidate`.
