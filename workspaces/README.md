...
Each named workspace commits its own `.gitignore`, exposing only its definition, documentation, task metadata and app deployment settings. Copy one of the maintained workspace ignore files when adding a definition; no source-root ignore exception is needed. Compose preserves this file, including local edits, in both linked and copied modes. Refresh releases older generated root ignore files from composition ownership and removes only unchanged generated app-level ignore files. Application source, `.env.example` files and credentials remain ignored. The parent `workspaces/.gitignore` guards uninitialized directories and retired scratch outputs; it is not an inventory of maintained workspace names.
