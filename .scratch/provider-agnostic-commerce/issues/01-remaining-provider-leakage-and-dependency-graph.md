...
1. Product Catalog and Store/assortment code exposes raw Commercetools filters, Product Projections, Product Selections, channel IDs, fragments, attribute encodings, and provider-shaped DTOs through Promise singletons.
...
Product Catalog/Discovery is the best next behavior slice, but it must include the provider-agnostic Product model and Store/assortment semantics. The physical package split is the last proof step. Moving files first would retain provider-shaped contracts or introduce a forbidden `@repo/commerce` to `@repo/commerce-commercetools` dependency.
