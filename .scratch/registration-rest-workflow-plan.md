...
`web -> apps/api REST -> registration services -> Vercel Workflow -> registration approval/rejection programs`.
...
- Complete the `registration` REST contract with:
  - `POST /registrations`
  - `GET /registrations`
...
- Add `apps/api/lib/registration-runtime.ts` for the live Effect layer.
- Use `b2b-registration-by-id` as the clean Effect-owned custom object container.
...
- Workflow approval/rejection steps run `registration` programs.
