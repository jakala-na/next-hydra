...
- [Commerce Context, Address Book, and Checkout seams](issues/02-current-address-book-and-checkout-seams.md) — Provide one request-scoped `CommerceContext`; the provider-selected `AddressBook` Layer captures it so application callers reuse trusted Buying Context without a parallel `CurrentAddressBook` Service.
- [Address Book domain and capability contract](issues/03-address-book-domain-and-capability-contract.md) — Use a universal `AddressBook` Service with schema-backed entries, address types/defaults, and concrete identity-free `list`, `get`, and reference-idempotent `save` operations; its Layer obtains the verified Customer principal from `CommerceContext`.
