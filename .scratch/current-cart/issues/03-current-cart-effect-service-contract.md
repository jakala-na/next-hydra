...
- `CurrentCart` exposes one named Effect Service method per buyer-facing Cart action, including static accessors for direct use at an Effect boundary. It does not rely on floating wrapper programs and does not expose a generic `change`, `execute`, command union, provider action list, patch, or arbitrary update method.
...
`CurrentCart` is a request-bound `Context.Service` with one named Effect Service method per buyer-facing Cart action:
...
This is the complete initial public method set. The Service exposes traced static accessors such as `CurrentCart.get()` and `CurrentCart.addItem(input)` so callers do not create pass-through Effect programs merely to access the Service. Do not replace these methods with `change`, `execute`, an intent union, arbitrary provider actions, a patch document, or a generic update method. New Cart behavior adds another named domain method only when a concrete buyer-facing use case requires it.
...
Allowed-source checks, Delivery Details orchestration, and Checkout Policy evaluation remain in `CheckoutSession`. `CommerceContext` owns the request principal and current-customer profile lookup. The selected `AddressBook` Layer captures that context and exposes identity-free `list`, `get`, and `save`; `CheckoutSession` consumes `AddressBook` for Delivery Details but does not own the address catalog. `CurrentCart` persists only the canonical Cart-owned values produced by Checkout orchestration.
...
- `addItem` is the only initial program that may create a missing Current Cart. For an anonymous buyer, creation sets the request's anonymous Cart ID before applying the item addition, so the Cart ID survives a later add failure. Repeating a successful or outcome-unknown `addItem` may add the quantity again; callers must not blindly retry it.
...
- legitimate Current Cart absence from selection, authorization, decoding, provider, and Current Cart operation failures;
...
- anonymous Cart ID `set` failure from Cart persistence failure;
...
- request-scoped Store and verified principal facts from `CommerceContext`, plus the private anonymous cookie `set`/`clear` seam established at the HTTP or Next request boundary.
