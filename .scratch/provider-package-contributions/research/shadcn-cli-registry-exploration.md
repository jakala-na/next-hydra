...
Status: evidence retained; the per-root Install Unit recommendation is superseded by [ShadCN root targets versus Next Hydra Install Units](06-shadcn-root-targets-vs-install-units.md) and the [canonical specification](../spec.md). Complete `~/...` workspace-root targets allow one ShadCN installation. The source citations and capability boundaries below remain useful historical evidence.
...
Consequently, a command run from `apps/web` cannot target `../drupal`; traversal is rejected. A universal exact-copy item run from the monorepo root can target both `apps/web/...` and `apps/drupal/...`, because both remain below the same `cwd`. That is the narrow mechanism behind the “yes for copying” answer.
...
- `registry:file` -> `apps/drupal/web/modules/custom/provider_bridge/provider_bridge.info.yml` (and further PHP/YAML files).
...
## Superseded follow-up: use ShadCN as the engine, not just inspiration

The decision to embed ShadCN remains current. The per-root Install Unit mechanics in this section were later disproved: the prototype's targets omitted their package and application prefixes. See the correction linked at the top of this document.

For the narrowed requirement—materialize copied code into developer workspaces and print the remaining setup—Next Hydra should **embed ShadCN rather than reimplement its registry protocol**. The current seam is one Next Hydra composition plan followed by one workspace-root ShadCN installation.
...
| Wrap/embed `shadcn/registry` | **Yes—recommended v1.** | It directly reuses the supported resolver and installer. The wrapper owns choices, compatibility, exact-artifact preparation, package-specific changes, and aggregated reporting around one workspace-root installation. |
...
Keep one Next Hydra-owned Composition Plan that selects registry items and validates the matrix before any write. Each item remains a standard universal ShadCN registry item, but every file declares its complete workspace-root target. The wrapper then calls the stable `addRegistryItems` API once from the workspace root.

This is intentionally more than a blind pass-through and much less than a new registry engine:

1. `create-next-hydra` selects CMS, Commerce, and add-ons and validates compatibility from their Selection Definitions before mutation.
2. It resolves and validates the complete intact registry-item graph.
3. It invokes `addRegistryItems` once from the workspace root with the trusted registry configuration and explicit overwrite policy.
4. ShadCN installs root npm dependencies, writes exact files, adds env/CSS contributions when applicable, and prints the item's setup docs.
5. The wrapper reports which later operation failed; atomic rollback is not promised.

The programmatic API does not expose the CLI's exact `--dry-run`/`--diff` surface—its options are `cwd`, config, overwrite behavior, silence, fonts, and path ([source](https://github.com/shadcn-ui/ui/blob/41bbc12cfd39ed8d9cb8da04275479ee7ecc0612/packages/shadcn/src/registry/add.ts#L12-L23)). For v1, the wrapper can show the requested item graph, resolved files and dependencies, and compatibility result through `getRegistryItems`/`resolveRegistryItems`, then install after confirmation. Customer `add` limits files to exact-copy `registry:file` and `registry:item` entries; framework-transformed file types are rejected because the programmatic API does not expose their exact prospective output for the wrapper's approval step.
...
Embed the documented `shadcn/registry` API in v1. Keep composition compatibility, exact-artifact preparation, package-specific manifest changes, binary assets, and maintainer-only removal in `create-next-hydra`; use universal items with complete workspace-root targets for exact copied source in one ShadCN installation. Forking or building a compatible resolver is future work only if requirements expand beyond those supported semantics.
