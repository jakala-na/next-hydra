...
- Keep schema parsing, Provider Slot cardinality, Selection ID checks, package-alias requirements, exact file-target conflict detection, and deterministic ordering inside that module. Let ShadCN resolve the registry dependency graph during installation.
...
- Use the real Drupal six-route and Contentstack two-route registry files in tests. Add focused fixtures only for invalid metadata and duplicate file targets.

Checkpoint: unit tests snapshot or compare the complete Drupal and Contentstack plans, prove stable ordering, and prove that invalid cardinality, missing definitions, unsafe targets, and duplicate target files fail before writes.
...
- Clone the starter into the requested missing or empty directory, remove the complete catalog-wide CMS target inventory, and install the selected CMS registry graph once from the workspace root through the documented ShadCN registry API.
- Apply the selected `@repo/cms` alias, remove `next-hydra.json`, the source registry manifests and `registry/` authoring directories, inactive Provider source, the generator package, and other existing maintainer-only paths, then run the root `pnpm install`.
...
Retain the prototype's deterministic planning, exact route paths, stable package-alias changes, and use of the documented `shadcn/registry` interface. Replace both its route generator and per-root installation mechanism with explicit registry files whose final workspace-root targets are installed by one root ShadCN invocation. Reuse the real Drupal and Contentstack route cases as production acceptance scenarios.
...
Slice one delivers CMS composition completely. A standard root ShadCN `registry.json` includes registry fragments beside the Drupal package, Drupal application, and Contentstack package. Provider-specific application routes live as explicit files under contribution-local `registry/` directories; there is no content-inlined generated registry output or route-generation language. One composition-planning module validates Selection Definitions, resolves the selected CMS, produces package-alias work, rejects file conflicts, and supplies the fresh-scaffold target inventory. ShadCN resolves the selected registry dependency graph and writes its explicit workspace-root targets in one root invocation. `create-next-hydra use --cms <selection>` changes only the Maintainer Workspace and `use --check` detects drift without writing. The existing create command gains CMS selection and reconstructs either CMS from the Baseline through the source registry. Both CMS scaffolds install and typecheck, and failed scaffold targets remain intact.
...
Slice three extends the same commands with compatibility-aware Add-ons and portable Presets. Realistic fixtures prove transitive requirements, a dependency graph targeting multiple workspace locations, compatible materialization, and pre-write rejection without publishing fake integrations. Slice four delivers customer `add` as a separate additive workflow that operates on current paths and contents rather than a receipt: it creates missing files, skips identical files, makes changed files explicit, and never deletes customer code.

The implementation retains the prototype's deterministic planning, exact route paths, package aliases, and documented ShadCN APIs. It models routes as explicit ShadCN registry files, replaces per-root installation with explicit workspace-root targets and one ShadCN dependency-graph installation, and discards the catch-all dispatcher, route generator, prototype `.mjs` code, rollback behavior, plan digest, destructive customer switching, and fake selections as production features. Each slice owns its end-to-end tests; a broader composition-matrix optimization policy is intentionally deferred until enough real Providers exist to make that decision useful.
