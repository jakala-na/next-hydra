...
Build the first end-to-end registration onboarding proof of concept in `@repo/registration-effect`. The slice should model reviewable registrations with Effect Schema, expose step-sized services with memory layers, and compose plain Effect programs for approving, rejecting, and accepting a registration approval invitation.
