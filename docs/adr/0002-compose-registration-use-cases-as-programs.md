...
Registration use cases are modeled as named Effect programs in `packages/registration-effect/programs/` that compose provider-independent capability services, not as new `Context.Service` classes by default. Entry adapters such as HTTP handlers, workflow entrypoints, server actions, jobs, and tests should stay thin: they decode, authorize, compose live layers, run programs, and map adapter-specific results, while Registration lifecycle policy and provider orchestration live in the programs.
