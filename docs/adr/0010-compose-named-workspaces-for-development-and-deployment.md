...
- Named workspaces may commit app-local deployment settings alongside their definitions and optional READMEs; those two files are no longer the only permitted inputs.
...
Within each named workspace, commit the definition, an optional README, and independently authored `apps/<app>/vercel.json` files for selected applications. Materialized source, manifests, dependencies, applied state, caches and credentials remain ignored. Deployment settings belong to the named workspace, not to registry refresh, and remain physical files.
